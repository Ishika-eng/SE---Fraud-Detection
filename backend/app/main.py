...
from app.api.routes import admin, auth_routes, status, otp
...
async def startup_db_client():
    await db.connect()
    # Gap 1 fix: hydrate ml_engine's in-memory phone hash set from MongoDB
    # so hashes survive a disk wipe of phone_hashes.json.
    from app.services.ml_service import ml_engine

    # Sync phone hashes: MongoDB → ml_engine (Gap 1 fix)
    db_hashes = await db.load_phone_hashes()
    if db_hashes:
        ml_engine.phone_hashes.update(db_hashes)
        logger.info(f"Synced {len(db_hashes)} phone hashes from MongoDB.")

    # Sync identity fingerprints: MongoDB → ml_engine (Layer 1 fix)
    db_fingerprints = await db.load_identity_fingerprints()
    if db_fingerprints:
        ml_engine.identity_fingerprints.update(db_fingerprints)
        logger.info(f"Synced {len(db_fingerprints)} identity fingerprints from MongoDB.")
...
app.include_router(otp.router,     prefix="/api", tags=["OTP — Phone Verification"])
